export default function ExpenseModal({open, onClose, children}){
    return (
        <div onClick={onClose} className={`fixed inset-0 flex justify-center items-center transition-colors ${open ? "visible bg-black/20": "invisible"}`}>
            ...
            <div onClick={e => e.stopPropagation()} className={`bg-white w-10/12 h-2/3 lg:w-1/2 lg:h-5/6 rounded-xl shadow p-4 transition-all text-black ${open ? "scale-100 opacity-100" : "scale-125 opacity-0"}`}>
